...
'''
nro_puntos = nro_puntos*nro_samples//10
nro_samples = 4

delays_real_test = np.ones((nro_puntos*nro_samples, (nro_mics*2)*(nro_mics*2 -1)//2))
delays_medidos_test = np.ones((nro_puntos*nro_samples, (nro_mics*2)*(nro_mics*2 -1)//2))
fuentes_pos_test = np.ones((nro_puntos*nro_samples, 3))
...
for i in range(nro_puntos):
	...
	while np.linalg.norm(fuente) > radio:
		...
		fuente = (np.random.rand(3)*2 - 1) *radio
	#Calculo los delays
	fuentes = np.ones((nro_samples,3))
	fuentes = fuentes*fuente
	tiempos = np.linalg.norm((tot_mics - fuentes), axis = 1)/350
	fuentes_pos_test[i*nro_samples:(i+1)*nro_samples] *= fuentes
	pos = 0
	for j in range(nro_mics*2):
		for k in range(j+1, nro_mics*2):
			delays_real_test[i*nro_samples:(i+1)*nro_samples, pos] *= tiempos[j] - tiempos[k]
			delays_medidos_test[i*nro_samples:(i+1)*nro_samples, pos] *= (tiempos[j] - tiempos[k]) + (np.random.rand(nro_samples)*2 -1) * 1/11000
			pos += 1
	
np.save('delays_real_test.npy', delays_real_test)
np.save('delays_medidos_test.npy', delays_medidos_test)
np.save('fuentes_pos_test.npy', fuentes_pos_test)
